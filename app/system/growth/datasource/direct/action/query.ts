...
import type { SchemaSearchParam, ColumnTable } from '@/app/system/growth/datasource/direct/action/schema'
...
import { LABEL, PATH, TABLE, TABLE_COLUMN_SORT } from '@/app/system/growth/datasource/direct/action/schema'
...
export const list = cache(async (params: SchemaSearchParam): Promise<Readonly<{ data : readonly ColumnTable[], metadata : SupabaseMeta}>> => {
	...
	const { page, limit, query, gclid, fbclid, status, purchase_pic, sort } = params
	...
	if (purchase_pic)
		supabaseQuery = supabaseQuery.ilike('purchase_pic', '%' + purchase_pic + '%')
...
export const sumPassedAmount = cache(async (): Promise<number> => {
	const supabase = await server()
	...
	const total = (data ?? []).reduce((sum, record) => {
		const amount = Number(record.purchase_amount) || 0
