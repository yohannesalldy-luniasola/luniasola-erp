...
import type { ColumnTable } from '@/app/system/growth/datasource/direct/action/schema'
...
import { remove, removeBatch }                                                                           from '@/app/system/growth/datasource/direct/action/mutation'
import { TABLE_COLUMNS, TABLE_COLUMN_INITIAL, TABLE_COLUMN_LOCKED, SCHEMA_SEARCH_PARAMS_INITIAL, LABEL } from '@/app/system/growth/datasource/direct/action/schema'
import { FormRemoval, FormUpdate }                                                                       from '@/app/system/growth/datasource/direct/component/form'
			...
			case 'purchase_amount':
				return (
					<Small className={'font-medium'}>
						{column.purchase_amount ? new Intl.NumberFormat('id-ID', {
							style                 : 'currency',
							currency              : 'IDR',
							minimumFractionDigits : 0,
							maximumFractionDigits : 0,
						}).format(Number(column.purchase_amount)) : '-'}
					</Small>
				)
			...
			case 'purchase_pic':
				return (
					<Small className={'text-neutral-600 dark:text-neutral-400'}>
						{column.purchase_pic && column.purchase_pic !== '' ? column.purchase_pic : '-'}
					</Small>
				)
