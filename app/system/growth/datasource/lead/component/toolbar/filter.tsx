...
export function ToolbarFilter() {
	...
	const performStatusChange = useCallback((value: string) => {
		...
		tableFilter.perform({ query : filterName || null, gclid : filterGclid || null, fbclid : filterFbclid || null, status : status || null })
	}, [ tableFilter, filterName, filterGclid, filterFbclid ])
	const performQueryKeyDown = useCallback((event: KeyboardEvent<HTMLInputElement>) => {
		if (event.key === 'Enter') {
			event.preventDefault()

			tableFilter.perform({ query : filterName || null, gclid : filterGclid || null, fbclid : filterFbclid || null, status : filterStatus || null })
		}
	}, [ tableFilter, filterName, filterGclid, filterFbclid, filterStatus ])

	const performClear  = useTableFilterClear({ filterName : setFilterName, filterGclid : setFilterGclid, filterFbclid : setFilterFbclid, filterStatus : setFilterStatus })
	const performFilter = useCallback(() => tableFilter.perform({ query : filterName || null, gclid : filterGclid || null, fbclid : filterFbclid || null, status : filterStatus || null }), [ tableFilter, filterName, filterGclid, filterFbclid, filterStatus ])

	return (
		<ToolbarFilterRoot filters={[ 'query', 'gclid', 'fbclid', 'status' ]} open={toolbarFilter.open} pending={tableFilter.pending} title={'Filter'} onClear={performClear} onFilter={performFilter} onOpenChange={toolbarFilter.setOpen}>
			<Fieldset>
				<Label>Name</Label>
