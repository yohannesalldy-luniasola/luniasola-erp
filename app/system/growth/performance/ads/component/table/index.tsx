...
export function Table({ data, account }: { readonly data : readonly ColumnTable[], readonly account : readonly { id : string, name : string }[] }) {
	...
	const performRenderCell = useCallback((column: ColumnTable, key: string) => {
		switch (key) {
			case 'channel': {
				if (column.channel === 'google')
					return (
						<Div className={'flex flex-row items-center gap-2.5'}>
							<SVG className={'size-4'} height={18.000} viewBox={'0 0 256 256'} width={18.000}>
							...
							<Small className={'font-medium text-neutral-900 dark:text-neutral-100'}>Google Ads</Small>
						</Div>
					)

				if (column.channel === 'facebook')
					return (
						<Div className={'flex flex-row items-center gap-2.5'}>
							<SVG className={'size-4'} height={18.000} viewBox={'0 0 256 256'} width={18.000}>
							...
							<Small className={'font-medium text-neutral-900 dark:text-neutral-100'}>Facebook</Small>
						</Div>
					)

				return (
					<Div className={'flex flex-row items-center gap-2.5'}>
						<Small className={'text-neutral-500'}>{column.channel || '-'}</Small>
					</Div>
				)
			}
